...
grid on
hold off
set(ax,'xlim',[6.25 6.29],'ylim',[-0.0025,0.0135])
...
for j=1:size(zhist,1)
  ...
  b_Fl = zeros(N,1);
  for i=1:N
    a_Fl(i) = zhist(j,1+i)*cos(i*omega*tau)-zhist(j,N+1+i)*sin(i*omega*tau);
    b_Fl(i) = zhist(j,1+i)*sin(i*omega*tau)+zhist(j,N+1+i)*cos(i*omega*tau);
  end
  a0_u2 = a0_Fl/k;
  a_u2  = zeros(N,1);
  b_u2  = zeros(N,1);
...
for i=1:size(zhist,1)
  [~, y] = hybridsensor_delay('',struct('N',N),zhist(i,:)');
  tol(i) = norm(y);
end
