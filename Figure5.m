...
%
% Script generating Figure 5 in Y. Mao, H. Dankowicz, "On a Principle for
...
prob = coco_add_event(prob, 'GP', 'gamma', 0.003:0.003:0.03);
bd1  = coco(prob, 'alg_run1', [], 1, ...
  {'gamma', 'beta', 'omega', 'u2c', 'u2s'}, [0 0.03]);
...
for i=1:length(GPlab1)
  ...
  coco(prob, ['alg_run1',num2str(i)], [], 1, ...
    {'delta','beta','omega', 'u2c', 'u2s'}, {[0 0.05] [-0.045 0.005]});
end
...
prob = coco_add_pars(prob, 'pars', 1:8, ...
  {'kappa', 'delta', 'gamma', 'beta', 'omega', 'u1c', 'u2c', 'u2s'});
prob = coco_add_event(prob, 'GP', 'gamma', [0.003 0.006]);
